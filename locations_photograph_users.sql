...
# Create the tables and define constraints and relationships
CREATE TABLE Author (
...
# Populate the Borrower table with data
...
# Select 
SELECT MONTHNAME(BorrowDate) AS MONTH, COUNT(BookID) As BorrowCount FROM BORROWER
...
SELECT
	(SELECT AuthorFirstName FROM Author WHERE Book.AuthorID = Author.AuthorID) AS AuthorFirstName,
    (SELECT AuthorLastName FROM Author WHERE Book.AuthorID = Author.AuthorID) AS AuthorLastName
FROM Book WHERE BookID IN (SELECT BOOKID FROM Borrower WHERE YEAR(BorrowDate) = 2017)
...
SELECT
(SELECT AuthorNationality FROM Author Where Book.AuthorID = Author.AuthorID) AS AuthorNationality
FROM BOOK
WHERE BookID IN (SELECT BookID FROM Borrower WHERE YEAR(BorrowDate) BETWEEN 2015 AND 2017)
GROUP BY AuthorNationality
ORDER BY COUNT(AuthorNationality) ASC
LIMIT 5;

# 6. The book that was most borrowed during the years 2015-2017
SELECT
(SELECT BookTitle FROM Book WHERE Borrower.BookID = Book.BookID) AS BookTitle
FROM Borrower
WHERE YEAR(BorrowDate) BETWEEN 2015 AND 2017
GROUP BY BookTitle ORDER BY COUNT(BookTitle) DESC
LIMIT 1;

# 7. Top borrowed genres for client born in years 1970-1980
# Used SQL JOIN (selecting from more than one table)
SELECT Book.Genre FROM Book, Borrower, Client WHERE Borrower.BookID = Book.BookID
AND Borrower.ClientID = Client.ClientID
AND Client.ClientDOB BETWEEN 1970 AND 1980
GROUP BY Genre ORDER BY COUNT(Genre) DESC
LIMIT 5;

# 7. Top borrowed genres for client born in years 1970-1980
# Used REGULAR JOIN / A.K.A. INNER JOIN (selecting from more than one table)
...
SELECT Occupation, COUNT(DISTINCT Client.ClientID) AS ClientCount FROM Client
JOIN Borrower ON Borrower.ClientID = Client.ClientID
WHERE YEAR(BorrowDate) = 2016
GROUP BY Occupation
...
# I need to practice this one (I got help online)
SELECT Client.Occupation, AVG(BookCount) FROM (SELECT ClientID, COUNT(BookID) AS BookCount FROM Borrower GROUP BY ClientID) AS BorrowCount
JOIN Client ON BorrowCount.ClientID = Client.ClientID
